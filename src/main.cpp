...
class $modify(PlayerObject) {
  ...
  void jumpscare() {
    ...
    if (Eduardo->getActionByTag(1)) {
      Eduardo->stopActionByTag(1);
    }

    // Ensure opacity is 255
    Eduardo->setOpacity(255);
    // Fade out robert
    Eduardo->runAction(CCFadeOut::create(1.0))->setTag(1);
  }
  ...
  void incrementJumps() {
    ...
    if (randomizeOption == false) {
        ...
        if (runningScene->getChildByID("eduardo") && isHolding) {
          this->jumpscare();
        }
    }
  }
  ...
  TodoReturn pushButton(PlayerButton p0) {
    ...
    if (!runningScene->getChildByID("eduardo")) {
      Eduardo = CCSprite::create("Eduardo.png"_spr);
      Eduardo->setID("eduardo");
      CCSize winSize = CCDirector::get()->getWinSize();
      
      float scaleRatio = (winSize.height / Eduardo->getContentSize().height);
      
      // Scale robert to fit screen
      Eduardo->setScaleX(scaleRatio);
      Eduardo->setScaleY(scaleRatio);
      
      // Center the robert
      Eduardo->setPosition({winSize.width / 2, winSize.height / 2});
      runningScene->addChild(Eduardo, 100);

      // Set robert opacity to 0
      Eduardo->setOpacity(0);
    }
